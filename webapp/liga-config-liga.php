...
if (isset($_GET['action'])) {
  if ($_GET['action'] == "configLiga") {
    ...
    if (isset($_SESSION['idLiga'])) {
      ...
      // Iniciar la sesion
      session_start();
      // Establecer la variable de sesi�n con el ID de la liga reci�n creada
      $_SESSION['idLiga'] = mysqli_insert_id($conn);
      if ($_SESSION['idLiga'] == 0) {
	    unset($_SESSION['idLiga']);
      }
    }
    // Sentencia SQL para borrar categor�as candidatas previas de la liga
    ...
    if (strlen($_POST['disciplinaPer']) != 0) {
      ...
      // Sentencia SQL para crear la categor�a candidata
      ...
      // Ejecutar la sentencia
      $rs = mysqli_query($conn,$ssql);
      // Tambi�n hay que establece a NULL la categor�a de la liga
      ...
      // Sentencia SQL para obtener el id num�rico de la categor�a
...
// Si se est� modificando una liga, se carga el formulario con sus par�metros
if (isset($_SESSION['idLiga']))
{
  // Obtener la configuraci�n de la liga
  ...
  // Sentencia SQL para obtener los datos de configuraci�n de la liga
  ...
  // Liberar el la memoria de los datos de configuraci�n de la liga
  mysqli_free_result($rs);
  ...
  if (mysqli_num_rows($rs) == 0) {
    // La liga no tiene una categor�a temporal, sino que tiene una categor�a existente
    ...
    // El nombre de la categor�a temporal se pone a vac�o
    $nombreCategoriaPer = "";
  }
  else {
    // El nombre de la categor�a a�n es temporal
...
// Presentar el formulario de creaci�n de una liga nueva
echo "
    <div id=\"navcontainer\">
      <ul id=\"navlist\">
